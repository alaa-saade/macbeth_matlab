...
/* The computational routine */
 void compute_current_estimate_vec_wjac(double *x, double *I, double *J,double *VAL,double n, double m, double r,double nnz,double *COST,double *JVAL)
 {
    ...
    for(int k=0;k<nnz;k++){
        ...
        for(int mu=1;mu<r+1;mu++){
            ...
            // JVAL[(int)(2*r*k+mu-1)] = 2.0*(-VAL[k] + xyt)*x[(int)(n*r+j+(mu-1)*m-1)];
            JVAL[(int)(2*r*k+mu-1)] = -x[(int)(n*r+j+(mu-1)*m-1)];
            // JVAL[(int)(2*r*k+r+mu-1)] = 2.0*(-VAL[k] + xyt)*x[(int)(i+(mu-1)*n-1)];
            JVAL[(int)(2*r*k+r+mu-1)] = -x[(int)(i+(mu-1)*n-1)];
        }
  ...
  int nrhs, const mxArray *prhs[])
{
    ...
    if(nlhs!=2) {
        mexErrMsgIdAndTxt("MyToolbox:arrayProduct:nlhs","2 outputs required: COST, JVAL");
    }
    ...
    plhs[1] = mxCreateDoubleMatrix(nnz*2*r,1,mxREAL);
    ...
    compute_current_estimate_vec_wjac(x,I,J,VAL,n,m,r,nnz,COST,JVAL);
}
